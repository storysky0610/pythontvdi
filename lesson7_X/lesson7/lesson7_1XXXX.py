...
class Window(ThemedTk):
    def __init__(self,*args, **kwargs):
        ...
        sitenames = datasource.get_sitename()
        self.selected_site = tk.StringVar()
        sitenames_cb = ttk.combobox(selectedFrame,textvariable=self.selected_site,values=sitenames,state='read')
        self.selected_site.set('請選擇城市')
        sitenames_cb.bind('<<ComboboxSelected>>',self.sitename_selected)
        selectedFrame.pack(side='left',expand = True,anchor="n")
       #============End SelectedFrame=====================  
    def county_selected(self,event):
        selected = self.selected_county
