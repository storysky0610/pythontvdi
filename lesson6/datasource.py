...
def get_sitenames()->list[str]:
    ...
    try:
        ...
        return sitenames
def get_seiected_data(sitename:str)->list[list]:
    ...
    try:
        ...
        for items in data['records']:
            if items['sitename'] == sitename:
                innerlist=[items['datacreationdata'],items['county'],items['aqi'],items['pm2.5'],items['status'],items['latitude'],items['longitude']]
